    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->save, &QAction::triggered, this, &MainWindow::save_to_file);
    connect(ui->open, &QAction::triggered, this, &MainWindow::open_file);
...
void MainWindow::save_to_file() {
    ...
    for (auto i : roads) {
        file << i->length << '\n' << i->first_city->name.toStdString() <<
               '\n' << i->second_city->name.toStdString() << '\n';
    }
    file.close();
}
...
void MainWindow::open_file() {
    ...
    while (file) {
        ...
        road *new_road = new road(first_city, second_city, l);
        roads.push_back(new_road);
        scene->addItem(new_road);
    }
    scene->update();
